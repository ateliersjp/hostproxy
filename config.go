...
func newConfig(m *http.Msg) (conf *config, ok bool) {
	...
	hostname, found := popHeader(m, "Host")
	...
	if found && len(ROOT_DOMAIN) != 0 {
		...
		if hostname, found = strings.CutSuffix(hostname, suffix); found {
			conf.repl = append(conf.repl, []byte(suffix), []byte{})

			hostname = strings.ReplaceAll(hostname, "--", ".")
			conf.host = fmt.Sprintf("%s%s", hostname, port)

			if scheme, _ := popHeader(m, SCHEME_HEADER); scheme == "https" || scheme == "on" {
				conf.tls = true
			}
	...
	if uri, found = strings.CutPrefix(uri, "https://"); found {
		...
		conf.tls = false
	} else if scheme, _ := popHeader(m, SCHEME_HEADER); scheme == "https" || scheme == "on" {
		conf.tls = true
	}
